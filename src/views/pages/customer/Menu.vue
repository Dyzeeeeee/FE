    ...
    const selectedServiceType = ref('Dine in'); // Default to 'Dine In'
    const myOrder = ref(0);
    const selectServiceType = async (typeId) => {
        selectedServiceType.value = typeId;
        if (orderId.value) {
            try {
                await axios.put(`/update-online-order/${orderId.value}`, { service: selectedServiceType.value });
            } catch (error) {
    ...
    async function checkOrderConfirmation() {
        try {
            ...
            if (response.data && response.data.confirmed == 1) {
                ...
                const response = await axios.get(`/get-latest-order`);
                myOrder.value = response.data.id;
            }
        } catch (error) {
                    ...
                    <div v-for="category in categories" :key="category.id" class="mb-4">
                        ...
                        <ul>
                            <li v-for="item in filteredMenuItems(category.id)" :key="item.id"
                                ...
                                <span class=" flex-1 pr-4  underline" @click="showDetails(item)">{{ item.name }}</span>
...
.fixed-bottom-overlay {
    position: fixed;
    /* Fixed positioning, makes the div stick to a position */
    bottom: 0;
    /* Aligns the div to the bottom of the viewport */
    left: 0;
    /* Aligns the div to the left of the viewport */
    width: 100%;
    /* Makes the div extend full width */
    ...
    /* A
    /* Ensures the div stays on top of other content */
}
</style>
