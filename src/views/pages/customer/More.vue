...
const feedbackTopic = ref(''); // Holds the selected topic
const feedbackDescription = ref(''); // Holds the feedback description
...
const sendFeedback = async () => {
    if (!feedbackTopic.value || !feedbackDescription.value) {
        alert('Please fill out all fields.');
        return;
    }
    try {
        // Example API call to send feedback
...
<template>
    ...
    <div class="grid grid-cols-12 gap-3 mt-9 -mx-5">
        ...
        <div class="col-span-6 lg:col-span-6 xl:col-span-8 ">
            ...
            <div class="card w-full pointer-cursor p-5" @click="visible = true">
                <div class="flex-wrap">
                    <div class="text-blue-400 mb-2">
                        <Icon icon="fluent:person-feedback-48-filled" height="30" width="30" />
                    </div>
                    <div class="self-center font-bold text-base">Send Feedback</div>
                </div>
            </div>
        </div>
    ...
    <div v-if="visible" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center z-50">
        <div class="bg-surface-800 rounded-lg p-6 w-11/12 max-w-lg">
            <h2 class="text-xl font-bold mb-4">Send Feedback</h2>
            <div class="mb-4">
                <label for="topic" class="block font-medium mb-1">Topic</label>
                <select id="topic" v-model="feedbackTopic" class="w-full border-gray-300 rounded-lg p-2">
                    <option disabled value="">Select a topic</option>
                    <option v-for="topic in feedbackTopics" :key="topic" :value="topic">
                        {{ topic }}
                    </option>
                </select>
            </div>
            <div class="mb-4">
                ...
                <textarea id="description" v-model="feedbackDescription" class="w-full border-gray-300 rounded-lg p-2"
                    rows="5" placeholder="Write your feedback here..."></textarea>
            </div>
            <div class="flex justify-end gap-3">
                <button class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg" @click="visible = false">
                    Cancel
                </button>
                <button class="bg-yellow-500 text-white px-4 py-2 rounded-lg" @click="sendFeedback">
                    Submit
                </button>
            </div>
        </div>
