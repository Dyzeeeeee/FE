<template>
    <div class="fixed bottom-0 left-0 right-0 bg-gray-800 text-white flex justify-evenly text-xs z-10 h-[4rem]">
        <!-- Home -->
        <div @click="$router.push('/admin/home')" class="flex-1 text-center p-2 cursor-pointer">
            <div :class="{ 'text-yellow-500 font-bold text-sm': $route.path === '/admin/home' }">Home</div>
            <div class="flex justify-center">
                <Icon icon="ion:home" width="20" height="20"
                    :class="{ 'text-yellow-500': $route.path === '/admin/home' }" />
            </div>
        </div>
        <div @click="$router.push('/admin/sales')" class="flex-1 text-center p-2 cursor-pointer">
            <div :class="{ 'text-yellow-500 font-bold text-sm': $route.path === '/admin/sales' }">Sales</div>
            <div class="flex justify-center">
                <Icon icon="icon-park-twotone:sales-report" width="20" height="20"
                    :class="{ 'text-yellow-500': $route.path === '/admin/sales' }" />
            </div>
        </div>

        <div @click="$router.push('/admin/more')" class="flex-1 text-center p-2 cursor-pointer">
            <div :class="{ 'text-yellow-500 font-bold text-sm': $route.path === '/admin/more' }">More</div>
            <div class="flex justify-center">
                <Icon icon="icon-park-outline:more-app" width="20" height="20"
                    :class="{ 'text-yellow-500': $route.path === '/admin/more' }" />
            </div>
        </div>
    </div>
