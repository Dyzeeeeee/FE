...
// Watch for changes in primary and surface and apply themes accordingly
watch(
    () => layoutConfig.primary,
    (newPrimary) => {
        const color = primaryColors.value.find((c) => c.name === newPrimary);
        if (color) {
...
watch(
    ...
    (newSurface) => {
        const surface = surfaces.value.find((s) => s.name === newSurface);
        if (surface) {
